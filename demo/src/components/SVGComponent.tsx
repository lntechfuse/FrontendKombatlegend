...
import Hexagon from "./Hexagon"; // ตรวจสอบให้แน่ใจว่า Hexagon รองรับ prop currentPlayer และ onDropHexagon
...
const SVGComponent: React.FC<SVGComponentProps> = ({
  ...
  // ฟังก์ชันสำหรับจัดการเมื่อมีการ drop minion ลงบน hexagon
  const handleHexagonDrop = (
    row: number,
    col: number,
    event: React.DragEvent<SVGElement>
  ) => {
    event.preventDefault();
    const minionType = event.dataTransfer.getData("text/plain");
    if (onHexagonDrop) {
      onHexagonDrop(row, col, minionType);
    }
  };
  ...
  return (
    ...
    >
      ...
      <Hexagon
        ...
        onDropHexagon={handleHexagonDrop}
      />
      <Hexagon
        row={1}
        col={2}
